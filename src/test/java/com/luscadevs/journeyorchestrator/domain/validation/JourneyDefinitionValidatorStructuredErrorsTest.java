...
class JourneyDefinitionValidatorStructuredErrorsTest {
    ...
    void shouldReturnStructuredErrorsWithCodeMessageAndField() {
        ...
        ValidationError journeyCodeError = errors.stream()
                .filter(e -> ValidationError.Codes.INVALID_BASIC_FIELDS.equals(e.getCode()))
                .filter(e -> "journeyCode".equals(e.getField()))
                .findFirst()
                .orElse(null);
        ...
        assertEquals(ValidationError.Codes.INVALID_BASIC_FIELDS, journeyCodeError.getCode());

        // Verificar erro de name
        ValidationError nameError = errors.stream()
                .filter(e -> ValidationError.Codes.INVALID_BASIC_FIELDS.equals(e.getCode()))
                .filter(e -> "name".equals(e.getField()))
                .findFirst()
                .orElse(null);
        ...
        ValidationError versionError = errors.stream()
                .filter(e -> ValidationError.Codes.INVALID_BASIC_FIELDS.equals(e.getCode()))
                .filter(e -> "version".equals(e.getField()))
                .findFirst()
                .orElse(null);
        
        assertNotNull(versionError);
        assertEquals("Journey version must be a positive integer", versionError.getMessage());
    ...
    void shouldReturnStructuredErrorForDuplicateState() {
        ...
        State duplicateState = State.builder().name("START").type(StateType.FINAL).build(); // Nome duplicado
    ...
    void shouldReturnStructuredErrorForUnreachableState() {
        // Arrange
        State initialState = State.builder().name("START").type(StateType.INITIAL).build();
        State reachableState = State.builder().name("REACHABLE").type(StateType.FINAL).build();
        State unreachableState = State.builder().name("UNREACHABLE").type(StateType.FINAL).build();
        
        Event event = Event.builder().name("PROCEED").description("Proceed").build();
        
        Transition transition = Transition.builder()
                .sourceState(initialState)
                .targetState(reachableState)
                .event(event)
                .build();
        ...
        assertEquals("State 'UNREACHABLE' is unreachable from initial state", error.getMessage());
        assertEquals("states", error.getField());
    }
