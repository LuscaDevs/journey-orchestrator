...
class JourneyDefinitionValidatorTransitionRulesTest {
    ...
    void shouldAcceptMultipleTransitionsWithConditions() {
        ...
        State approvedState = State.builder().name("APPROVED").type(StateType.FINAL).build();
        State rejectedState = State.builder().name("REJECTED").type(StateType.FINAL).build();
        
        Event evaluateEvent = Event.builder().name("EVALUATE").description("Evaluate request").build();
        
        // Múltiplas transições com mesmo (state + event) mas com conditions diferentes
        Transition highScoreTransition = Transition.builder()
                .sourceState(initialState)
                .targetState(approvedState)
                .event(evaluateEvent)
                .condition("#eventData.score >= 700")
                .build();
                
        Transition lowScoreTransition = Transition.builder()
                .sourceState(initialState)
                .targetState(rejectedState)
                .event(evaluateEvent)
                .condition("#eventData.score < 700")
                .build();
        ...
        JourneyDefinition definition = JourneyDefinition.builder()
                .id("test-id")
                .journeyCode("EVALUATION_JOURNEY")
                .name("Evaluation Journey")
                .version(1)
                .active(true)
    ...
    void shouldRejectMultipleTransitionsWithoutCondition() {
        ...
        // Act & Assert
        JourneyDefinitionValidationException exception = assertThrows(
                JourneyDefinitionValidationException.class,
                () -> validator.validate(definition)
        );
        
        assertTrue(exception.hasErrorsWithCode(ValidationError.Codes.AMBIGUOUS_TRANSITION));
        assertTrue(exception.getErrors().get(0).getMessage().contains("Ambiguous transition"));
    }
    ...
    void shouldRejectMixedTransitionsWithAndWithoutCondition() {
        ...
        Transition defaultTransition = Transition.builder()
                .sourceState(initialState)
                .targetState(approvedState)
                .event(evaluateEvent)
                .build(); // Sem condition (fallback)
                
        Transition conditionalTransition = Transition.builder()
                .sourceState(initialState)
                .targetState(rejectedState)
                .event(evaluateEvent)
                .condition("#eventData.score < 500")
                .build(); // Com condition
    ...
    void shouldAcceptSingleTransitionWithoutCondition() {
        ...
        Transition transition = Transition.builder()
                ...
                .event(proceedEvent)
                .build(); // Única transição, sem condition é OK
    ...
    void shouldAcceptMultipleTransitionsWithDifferentEvents() {
        ...
        Transition approveTransition = Transition.builder()
                .sourceState(initialState)
                .targetState(approvedState)
                .event(approveEvent)
                .build(); // Sem condition
                
        Transition rejectTransition = Transition.builder()
                .sourceState(initialState)
                .targetState(rejectedState)
                .event(rejectEvent)
                .build(); // Sem condition
    ...
    void shouldAcceptComplexTransitionsWithMultipleConditions() {
        // Arrange
        State initialState = State.builder().name("PENDING").type(StateType.INITIAL).build();
        State approvedState = State.builder().name("APPROVED").type(StateType.FINAL).build();
        State rejectedState = State.builder().name("REJECTED").type(StateType.FINAL).build();
        State manualState = State.builder().name("MANUAL_REVIEW").type(StateType.INTERMEDIATE).build();
        ...
        Transition mediumScoreTransition = Transition.builder()
                .sourceState(initialState)
                .targetState(approvedState)
                .event(evaluateEvent)
        ...
        Transition manualTransition = Transition.builder()
                .sourceState(initialState)
        ...
        JourneyDefinition definition = JourneyDefinition.builder()
                .id("test-id")
                .journeyCode("COMPLEX_JOURNEY")
                .name("Complex Journey")
                .version(1)
                .active(true)
                .initialState(initialState)
                .states(List.of(initialState, approvedState, rejectedState, manualState))
                .transitions(List.of(highScoreTransition, mediumScoreTransition, lowScoreTransition, manualTransition))
                .createdAt(Instant.now())
                .build();
