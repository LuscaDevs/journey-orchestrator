...
public class JourneyDefinitionController implements JourneysApi {
    ...
    public ResponseEntity<List<JourneyDefinitionResponse>> getJourneyDefinitionsByCode(@NotNull String journeyCode) {
        List<JourneyDefinition> definitions = journeyDefinitionService.getJourneyDefinitionsByCode(journeyCode);
        List<JourneyDefinitionResponse> responses = definitions.stream()
                .map(JourneyDefinitionMapper::toResponse)
                .toList();
        ...
        List<JourneyDefinitionResponse> responses = definitions.stream()
                .map(definition -> {
                    try {
                        ...
                        logger.error("Error mapping journey definition: {} - {}", definition.getJourneyCode(),
                                e.getMessage(), e);
                        throw e;
                    }
                })
