...
/**
 * Global exception handler for centralized error processing across all REST controllers. Provides
 * RFC 9457-compliant error responses with proper HTTP status mapping.
 */
...
public class GlobalExceptionHandler {
        ...
        public ResponseEntity<ProblemDetail> handleDomainException(DomainException ex,
                ...
                log.warn("Domain exception occurred: {} - {} | Path: {} | Context: {} | CorrelationId: {}",
                                ex.getErrorCode().getCode(), ex.getDetails(),
                                request.getRequestURI(), sanitizeContext(ex.getContext()),
                                correlationId, ex);
                ...
                return ResponseEntity.status(errorResponse.getProblemDetail().getStatus())
        ...
        public ResponseEntity<ProblemDetail> handleJourneyDefinitionNotFoundException(
                        JourneyDefinitionNotFoundException ex, HttpServletRequest request) {

                String correlationId = getOrCreateCorrelationId(request);
                setupLoggingContext(correlationId, ex, request);

                log.info("Journey definition not found: {} | Path: {} | Requester context: {} | CorrelationId: {}",
                                ex.getJourneyDefinitionId(), request.getRequestURI(),
        ...
        public ResponseEntity<ProblemDetail> handleJourneyDefinitionAlreadyExistsException(
                        JourneyDefinitionAlreadyExistsException ex, HttpServletRequest request) {
        ...
        public ResponseEntity<ProblemDetail> handleJourneyInstanceNotFoundException(
                        JourneyInstanceNotFoundException ex, HttpServletRequest request) {

                String correlationId = getOrCreateCorrelationId(request);
                setupLoggingContext(correlationId, ex, request);

                log.info("Journey instance not found: {} | Path: {} | Requester context: {} | CorrelationId: {}",
                                ex.getJourneyInstanceId(), request.getRequestURI(),
                                sanitizeContext(ex.getContext()), correlationId);
        ...
        public ResponseEntity<ProblemDetail> handleInvalidStateTransitionException(
                ...
                log.warn("Invalid state transition attempted: {} -> {} for instance {} | Path: {} | Context: {} | CorrelationId: {}",
                                ex.getFromState(), ex.getToState(), ex.getJourneyInstanceId(),
                                request.getRequestURI(), sanitizeContext(ex.getContext()),
                                correlationId);
        ...
        public ResponseEntity<ProblemDetail> handleJourneyAlreadyCompletedException(
                        JourneyAlreadyCompletedException ex, HttpServletRequest request) {
        ...
        public ResponseEntity<ProblemDetail> handleGenericException(Exception ex,
                ...
                problemDetail.setType(URI
                                .create("https://api.journey-orchestrator.com/errors/system_001"));
         ...
         */
        private void setupLoggingContext(String correlationId, DomainException ex,
                        HttpServletRequest request) {
                MDC.put(CORRELATION_ID_MDC_KEY, correlationId);
                MDC.put("requestPath", request.getRequestURI());
                MDC.put("httpMethod", request.getMethod());
        ...
        private boolean isSensitiveKey(String key) {
                ...
                return lowerKey.contains("password") || lowerKey.contains("token")
