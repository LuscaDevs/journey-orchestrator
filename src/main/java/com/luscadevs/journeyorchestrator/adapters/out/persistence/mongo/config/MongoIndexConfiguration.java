...
public class MongoIndexConfiguration {
    ...
    private void createJourneyDefinitionIndexes() {
        ...
        mongoTemplate.indexOps("journey_definitions")
                .createIndex(new Index().on("journeyCode", Sort.Direction.ASC)
                        .on("version", Sort.Direction.ASC).named("idx_journey_code_version_unique")
                        .unique());

        // Index on name for search
        mongoTemplate.indexOps("journey_definitions")
                .createIndex(new Index().on("name", Sort.Direction.ASC).named("idx_name"));

        // Index on active status
        mongoTemplate.indexOps("journey_definitions")
                .createIndex(new Index().on("active", Sort.Direction.ASC).named("idx_active"));
    }

    private void createJourneyInstanceIndexes() {
        // Index on journey definition ID
        mongoTemplate.indexOps("journey_instances").createIndex(new Index()
                .on("journeyDefinitionId", Sort.Direction.ASC).named("idx_journey_definition_id"));

        // Index on current state
        mongoTemplate.indexOps("journey_instances").createIndex(
                new Index().on("currentState", Sort.Direction.ASC).named("idx_current_state"));

        // Index on status
        mongoTemplate.indexOps("journey_instances")
                .createIndex(new Index().on("status", Sort.Direction.ASC).named("idx_status"));

        // Compound index on journey definition ID and status
        mongoTemplate.indexOps("journey_instances")
                .createIndex(new Index().on("journeyDefinitionId", Sort.Direction.ASC)
                        .on("status", Sort.Direction.ASC)
                        .named("idx_journey_definition_id_status"));

        // Index on started at for time-based queries
        mongoTemplate.indexOps("journey_instances").createIndex(
                new Index().on("startedAt", Sort.Direction.ASC).named("idx_started_at"));

        // Index on last activity at for activity tracking
        mongoTemplate.indexOps("journey_instances").createIndex(
                new Index().on("lastActivityAt", Sort.Direction.ASC).named("idx_last_activity_at"));

        // TTL index for completed instances (optional - 30 days)
        mongoTemplate.indexOps("journey_instances")
                .createIndex(new Index().on("completedAt", Sort.Direction.ASC)
                        .named("idx_completed_at_ttl").expire(30 * 24 * 60 * 60)); // 30 days in
                                                                                   // seconds
    }

    private void createEventIndexes() {
        // Index on journey instance ID
        mongoTemplate.indexOps("journey_events").createIndex(new Index()
                .on("journeyInstanceId", Sort.Direction.ASC).named("idx_journey_instance_id"));

        // Index on event type
        mongoTemplate.indexOps("journey_events").createIndex(
                new Index().on("eventType", Sort.Direction.ASC).named("idx_event_type"));

        // Compound index on journey instance ID and event type
        mongoTemplate.indexOps("journey_events")
                .createIndex(new Index().on("journeyInstanceId", Sort.Direction.ASC)
                        .on("eventType", Sort.Direction.ASC)
                        .named("idx_journey_instance_id_event_type"));

        // Index on timestamp for time-based queries
        mongoTemplate.indexOps("journey_events").createIndex(
                new Index().on("timestamp", Sort.Direction.ASC).named("idx_timestamp"));
        ...
        mongoTemplate.indexOps("journey_events")
                .createIndex(new Index().on("previousState", Sort.Direction.ASC)
                        .on("newState", Sort.Direction.ASC).named("idx_state_transition"));

        // TTL index for events (optional - 90 days)
        try {
            mongoTemplate.indexOps("journey_events")
                    .createIndex(new Index().on("timestamp", Sort.Direction.ASC)
                            .named("idx_timestamp_ttl").expire(90 * 24 * 60 * 60)); // 90 days in
                                                                                    // seconds
        } catch (Exception e) {
